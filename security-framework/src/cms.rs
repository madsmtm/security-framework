...
use objc2_core_foundation::{CFArray, CFData, CFAbsoluteTime, CFString};
use objc2_security::*;

use crate::base::{OSStatus, Result};
...
mod encoder {
    ...
    bitflags::bitflags! {
        ...
        pub struct SignedAttributes: u32 {
            /// Identify signature, encryption, and digest algorithms supported by the encoder
            const SMIME_CAPABILITIES = CMSSignedAttributes::AttrSmimeCapabilities.0;
            /// Indicate that the signing certificate included with the message is the preferred one for S/MIME encryption
            const SMIME_ENCRYPTION_KEY_PREFS = CMSSignedAttributes::AttrSmimeEncryptionKeyPrefs.0;
            /// Indicate that the signing certificate included with the message is the preferred one for S/MIME encryption,
            /// but using an attribute object identifier (OID) preferred by Microsoft
            const SMIME_MS_ENCRYPTION_KEY_PREFS = CMSSignedAttributes::AttrSmimeMSEncryptionKeyPrefs.0;
            /// Include the signing time
            const SIGNING_TIME =  CMSSignedAttributes::AttrSigningTime.0;
            /// Include Apple codesigning hash agility
            const APPLE_CODESIGNING_HASH_AGILITY = CMSSignedAttributes::AttrAppleCodesigningHashAgility.0;
            /// Include Apple codesigning hash agility, version 2
            const APPLE_CODESIGNING_HASH_AGILITY_V2 = CMSSignedAttributes::AttrAppleCodesigningHashAgilityV2.0;
            /// Include the expiration time
            const APPLE_EXPIRATION_TIME = CMSSignedAttributes::AttrAppleExpirationTime.0;
        }
    }
    ...
    impl CMSEncoder {
        ...
        pub fn create() -> Result<Self> {
            let mut inner: *mut objc2_security::CMSEncoder = ptr::null_mut();
        ...
        pub fn set_signer_algorithm(&self, digest_algorithm: &str) -> Result<()> {
            ...
            cvt(unsafe { CMSEncoderSetSignerAlgorithm(&self.0, alg) })?;
            Ok(())
        }
        ...
        pub fn add_signers(&self, signers: &[SecIdentity]) -> Result<()> {
            ...
            cvt(unsafe {
                CMSEncoderAddSigners(
                    &self.0,
                    if signers.is_empty() { ptr::null() } else { signers.as_CFType() })
            })?;
            Ok(())
        }
        ...
        pub fn get_signers(&self) -> Result<Vec<SecIdentity>> {
            let mut out: &CFArray = ptr::null_mut();
        ...
        pub fn add_recipients(&self, recipients: &[SecCertificate]) -> Result<()> {
            ...
            cvt(unsafe {
                CMSEncoderAddRecipients(
                    &self.0,
                    if recipients.is_empty() { ptr::null() } else { recipients.as_CFType() })
            })?;
            Ok(())
        }
        ...
        pub fn set_encapsulated_content_type_oid(&self, oid: &str) -> Result<()> {
            ...
            cvt(unsafe { CMSEncoderSetEncapsulatedContentTypeOID(&self.0, oid.as_CFType()) })?;
            Ok(())
        }
        ...
        pub fn get_encapsulated_content_type(&self) -> Result<Vec<u8>> {
            let mut out: &CFData = ptr::null_mut();
        ...
        pub fn add_supporting_certs(&self, certs: &[SecCertificate]) -> Result<()> {
            ...
            cvt(unsafe {
                CMSEncoderAddSupportingCerts(
                    &self.0,
                    if !certs.is_empty() { certs.as_CFType() } else { ptr::null() })
            })?;
            Ok(())
        }
        ...
        pub fn get_signer_timestamp_with_policy(
            &self,
            timestamp_policy: Option<&CFString>,
            signer_index: usize,
    ...
    pub fn cms_encode_content(
        ...
        cvt(unsafe {
            CMSEncodeContent(
                if signers.is_empty() { ptr::null() } else { signers.as_CFType() },
                if recipients.is_empty() { ptr::null() } else { recipients.as_CFType() },
                content_type_oid.as_ref().map(|oid| oid.as_CFType()).unwrap_or(ptr::null()),
...
mod decoder {
    ...
    impl CMSDecoder {
        ...
        pub fn create() -> Result<Self> {
            let mut inner: *mut objc2_security::CMSDecoder = ptr::null_mut();
        ...
        pub fn set_detached_content(&self, detached_content: &[u8]) -> Result<()> {
            ...
            cvt(unsafe { CMSDecoderSetDetachedContent(&self.0, data) })?;
            Ok(())
        }
        ...
        pub fn get_signer_status(
            ...
            let mut sec_trust: *mut objc2_security::SecTrust = ptr::null_mut();
            ...
            cvt(unsafe {
                CMSDecoderCopySignerStatus(
                    &self.0,
                    signer_index,
                    if policies.is_empty() { ptr::null() } else { policies.as_CFType() },
                    true.into(),
                    &mut signer_status,
                    &mut sec_trust,
                    &mut verify_result,
        ...
        pub fn get_signer_email_address(&self, signer_index: usize) -> Result<String> {
            let mut out: &CFString = ptr::null_mut();
...
mod tests {
    ...
    use crate::policy::SecPolicy;
    use objc2_security::CMSSignerStatus;
